...
f_c   = 77e9;	 % Carrier Freq.
delta_r = 1;     % Range Resolution
V_max = 70;      % Max. Velocity
range_max = 200; % Max. Range

Bsweep = c / 2 * delta_r;    %Bandwidth (B) 
T_s = 5.5*(range_max*2/c);   %chirp time
slope  = Bsweep/T_s;              %slope
...
% chirp
t=linspace(0,Nd*T_s,Nr*Nd); %total time for samples
...
sig_fft = abs(sig_fft/max(max(sig_fft)));
 % *%TODO* :
% Output of FFT is double sided signal, but we are interested in only one side of the spectrum.
% Hence we throw out half of the samples.
sig = sig_fft(1:length(t)/2+1);   % Half of output
